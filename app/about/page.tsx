...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description:
      "Command nations across all of history with an AI Game Master. Type orders in plain English; the AI adjudicates the consequences.",
  },
};
...
export default function AboutPage() {
  return (
    // The game sets `body { overflow: hidden }`; this wrapper restores
    // scrolling for the marketing page only.
    <div className="h-screen overflow-y-auto bg-[#0B0F19] text-slate-200">
        ...
        <ol className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
          {STEPS.map((s) => (
            <li
              ...
              className="rounded-xl border border-slate-800 bg-[#151B2B] p-5"
            >
              <div className="font-mono text-xs tracking-[0.15em] text-amber-500/70">
                {s.n}
              </div>
      ...
      <section className="mx-auto max-w-3xl px-5 py-12">
        ...
        <p className="mt-2 text-sm leading-7 text-slate-300">
          ...
          Gemini, DeepSeek, or a local OpenAI-compatible endpoint. Your API key
          stays in your session and is sent only to the provider you chose.
        </p>

        <h2 className="mt-8 text-xl font-semibold text-amber-500">
          Saves &amp; alternate timelines
        </h2>
        <p className="mt-2 text-sm leading-7 text-slate-300">
          Cloud save when signed in; rewind to any prior turn and branch a new
          timeline from there. Saves serialize only province ownership, so the
          format survives changes to the underlying map data.
        </p>

        {/* Experiments / prototypes — small callout per StoryTunes task */}
        <h2 className="mt-8 text-xl font-semibold text-amber-500">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-300">
          ...
          <li>
            A simulation with hard combat math — the LLM is the rules engine,
            with strict-shape JSON as the contract.
          </li>
          <li>
            A chat-with-NPC product. The AI is the world referee, not a
            character.
          </li>
        </ul>
      </section>
